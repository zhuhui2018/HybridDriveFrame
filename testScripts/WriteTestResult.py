...
def writeTestResult(sheetObj,rowNo,colsNo,testResult,errorInfo=None,picPath=None):
    ...
    try:
        # 在测试步骤sheet中，写入测试结果,colsNo代表的是sheet类型：testCase/testStep/dataSheet三者之一
        ...
        if errorInfo and picPath:
            "在测试步骤sheet中，写入异常信息"
            excelObj.writeCell(sheetObj,content=errorInfo,rowNo=rowNo,closNo=testStep_errorInfo)
